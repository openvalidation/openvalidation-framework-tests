...
public class GreaterOrEqualTests {
    ...
    // int
    @ParameterizedTest
    @ValueSource(ints = {124, Integer.MAX_VALUE})
    void int_int_expected_false(int other) {
        int input = 123;
        Assertions.assertFalse(huml.GREATER_OR_EQUALS(input, other));
    }
    ...
    void int_int_expected_true(int other) {
        int input = 123;
        Assertions.assertTrue(huml.GREATER_OR_EQUALS(input, other));
    }
    
    @ParameterizedTest
    @ValueSource(doubles = {123.000000000001, 123.1, 654.321, Double.MAX_VALUE})
    ...
    // double
    @ParameterizedTest
    @ValueSource(ints = {1, 123, Integer.MAX_VALUE})
